...
function addtoCartClicked(event){
    ...
    if(carritoCompras != null){
        ...
        if(productoDuplicado == undefined){
            ...
            return;
        }else if(productoDuplicado.nombre == itemTitle){
            console.log(productoDuplicado.nombre);
            productoDuplicado.sumarCantidad(productoDuplicado.cantidad);
            guardarStorage('Productos', carritoCompras);
            addItemToShoppingCart(itemTitle, itemPrice, itemImage, productoDuplicado.cantidad)
            return;
        }
...
function addItemToShoppingCart(title, price, image, cantidad){
    ...
    <div class="content-product-car">
        ...
        <div class="masCantidad">
            <input type="number" style="width:38px; margin-top:20px " value="${cantidad}" class="cantidadProducto">
        </div>
